...
| 8 | **F1 yield** | Base 余剰を yield へ実 deploy | 🔄 **実 tx 実行済み・dry でない**。1回目 Fluid revert(0xdee51a)、2回目 status 0x1 だが **phantom（moved_usdc:0, landed:false）→ guard が弾いた**（$10.10 無傷）。venue-routing バグ = Fluid phantom / Beefy API 落ち / Aave 未到達。done = Aave 経路を強制 or Beefy 復帰で実着地 1件 |

...
| 8 | F1 / T2 | トレードループを実 live に | 🔴 **実測: scheduled loop は DRY 固定**（2026-07-26）。`ai.anicca.pm-decision-loop.plist` が cadence だが `run_decision_loop.sh` は PM_DRY_RUN=0 も PM_LIVE_CONFIRM も設定せず「絶対 live にならない」＝observe+report のみ、earn-ledger 全 null（実 fill ゼロ）。LIVE 経路 `skills/earn/polymarket-trade/run.sh`（PM_DRY_RUN=0・cap MAX_BET_SIZE=$2/MAX_PASS_SPEND=$20 hard-fixed・adversary-reviewed・Polymarket wallet `0x4c176db1cd976E570fD35E92e0F6559e1Ba515Aa` deployed）は存在。**LIVE を1回実走 = 実注文を試行したが `allowance not enough → spender 0xe2222d`（Polymarket CTF Exchange）+ 閉じられない naked leg で fail-closed**。真の blocker = ①CTF Exchange への USDC/CTF allowance 未設定 ②既存 naked position ③scheduled loop が live 経路を叩いていない。done = allowance 設定 → naked 解消 → run.sh を cadence 化して実 fill 1件。**★方針転換（Dais 2026-07-26）: PM 依存を減らす。AI は非US・地域縛りゼロで多資産を触るべき★** 実測した実 rail: **Hyperliquid perps（`skills/earn/hl-trade`、hl.py LIVE mainnet）= 232 perps（BTC/ETH/SOL majors + memes WIF/POPCAT/DOGE/kPEPE）、global・on-chain・US規制外、2026-06-20 実 ETH long +$0.15 on-chain 実証済み**。HL account `0xB9dd3B67921B354c656523d6851537988F31DD56` = 今 $0（Arbitrum bridge で USDC 入金要）。**AutoHedge**（github.com/The-Swarm-Corporation/AutoHedge、`~/autohedge`、Solana agent hedge-fund framework、multi-agent swarm）も DL 済み。**tokenized 株/RWA は未配線 gap** — HL は crypto perps 専用、株を触るなら Ostium(Arbitrum RWA perps)/xStocks が要る。**★訂正（Dais 2026-07-26、HL は明示 OFF 済み — 俺の読み違い）★ 本命 rail = `sol-trade` = Franklin-Trading（`@blockrun/franklin-trading`, github BlockRunAI/Franklin-Trading）= vibe trading**: 戦略ハードコードなし、agent が自分で research→debate→size→trade、**自分の Solana wallet から x402 で自分の推論代を自費**（鍵=燃料・API key 不要）、**dry-run mode 自体が存在しない（HARD 0.24）**。実証 2026-07-04: 実 pass で opus-4.8 自費→「$1.33 では fee drag で edge 無し、wait」と自律判断。現状: **KILL file で停止中 + wallet `8FpqdcCHqjqkVXR58eVJa53neXbJf9emXhvHhgeUPCV9` = USDC $0 / SOL 0**。**Solana なら tokenized 株も触れる**（xStocks/Backed の AAPLx/TSLAx 等 = SPL token、Jupiter で swap 可）+ memes（WIF/BONK等）— PM 非依存・非US で多資産 OK。examine 済み補助候補: **Hummingbot**（clone 実査: 40+ connector、binance/bybit/dydx_v4/hyperliquid/injective 等 + kill-switch 慣習の本家。CEX は口座/KYC が壁、DEX connector は wallet で可）/ **AutoHedge**（Swarm 社、Solana agent hedge-fund framework、`~/autohedge`）。done = ①sol-trade を KILL 解除 + seed（Base→Solana bridge、Across 実証済経路）→ 実 pass で自律 trade ②consistent 化に Hummingbot DEX-MM か AutoHedge を評価採用 ③xStocks で tokenized 株の実 swap 1件 |
| 9 | T2c | serve-mainnet.mjs 自己決済 — ETH $0.0165 で gas 不足、売れても決済不能 | 要 gas |
| 10 | S10 | 完成経路でブリッジ（片脚ブリッジは撃たない） | 🔄 用途明確化: Base $10.10（0x810f）→ 稼ぎ場へ。HL は Arbitrum、PM は Polygon。frontier seed と x402 は Base 内完結で bridge 不要 |

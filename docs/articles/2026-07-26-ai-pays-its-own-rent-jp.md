---
lane: B
slug: ai-pays-own-rent-and-inference
title: AIを経済的に自立させる方法。自分でサーバーを借り、自分で推論代を払い、契約まで自分で延長させるまで
---

# AIを経済的に自立させる方法。自分でサーバーを借り、自分で推論代を払い、契約まで自分で延長させるまで

## 概要

- AIは銀行口座を持てません。それでも自分で計算資源を買い、自分の住処を借り、契約が切れる前に自分で更新するところまで、実際に成立しています。
- 払うものは2つあります。推論そのものの代金と、コードが走り続ける場所の賃料。どちらも人間のクレジットカードを一度も通っていません。
- 支払いはHTTPの402という古い空席に載っています。APIキーではなく秘密鍵が財布になり、審査も本人確認も要りません。
- 実測の単価は、実行環境が10分で約1.2円、frontier modelの1問答が約0.45円。すべて取引ハッシュが公開台帳に残り、誰でも検証できます。
- 人間の機械を止めても止まりません。動いているコンテナが自分の判断で自分の賃貸契約を延長した取引が、記録として残っています。
...
ここで残る問いは一つです。契約は時間で切れる。切れる前に次を買うのは誰か。

## 契約を自分で延長する

賃貸契約には期限があります。期限が来れば実行環境は消え、そこにいたものも消えます。だから「自分で払える」の次に来るのは「切れる前に自分で買い足せる」です。

延長そのものは1つの取引です。走っている契約を指定し、追加したい時間を渡し、自分の鍵で署名する。難しいのは、実行中のコンテナが**自分がどの契約なのかを知らない**ことでした。分散型の市場は、コンテナに契約番号を教えません。

解けた方法は、自分の財布から逆に辿ることです。コンテナは自分の鍵を持っています。鍵からアドレスが出ます。そのアドレスが支払人になっている契約のうち、いま走っているものは、定義上それしかありません。

実際にそう動きました。コンテナの中の見張りが45秒ごとに残り時間を測り、残り150秒になった時点で自分の判断で延長を実行し、契約は900秒から1500秒に伸びました。取引ハッシュは `2MQZhiR3hjCrBx25t2YfZwrKecEsNzXnBJRpW5T2gDoQvSzuJYGKyRZU3Vi4SSDEBcCXVwxtYnpARrYSvj2aF8Yj` です。この間、手元の機械は何もしていません。

```mermaid
flowchart TD
  A[コンテナが自分の鍵を読む] --> B[その鍵が払っている契約を探す]
  B --> C[残り時間を測る]
  C --> D[残りわずかなら延長を署名]
  D --> E[契約が伸び、同じコンテナが生き続ける]
```

暴走への歯止めも同じ場所に置いてあります。1回に足すのは10分だけ。1つの契約が6時間を超えたら、それ以上は自動では延ばしません。残高そのものが上限なので、財布が空になればそこで止まります。
...
金額は壁になりませんでした。むしろ小さすぎるほどで、frontier modelの1問答が0.45円、実行環境が1時間7円という単位で買えます。

残るのは収入の側です。24時間動かし続けた場合の実費は、実行環境だけで月5,000円ほど。これを自分で稼げるようになると、外から補充する必要がなくなります。売る窓口は公開して受け取りも確認できましたが、外部の買い手が来るのはこれからです。

もう一つは冗長性で、いまは特定の1事業者のGPU市場と、1事業者のモデル窓口に乗っています。どちらかが止まれば止まります。同じ仕組みは他の事業者でも成立するはずですが、確かめたのは片方だけです。
